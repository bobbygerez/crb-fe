...
<div class="q-pa-sm">
        ...
        <div class="col-xs-9">
            <q-card inline class="full-width">
                ...
                <q-card-main>
                    <div class="row">
                        <div class="col-xs-4">
                            <q-select v-model="payee.payable_type" filter :options="entities" float-label="Vendor Type"  class="q-ml-sm" />
                        </div>
                        <div class="col-xs-4">
                            <q-select v-model="payee.payable_id" filter :options="vendorableNames" float-label="Vendor Name"  class="q-ml-sm" />
                        </div>
                        <div class="col-xs-4">
        ...
        <div class="col-xs-3">
            <div class="col-xs-8">
                <q-card inline class="q-ml-sm">
                    <q-card-main>
                        <q-select v-model="transaction.chart_account_id" filter :options="chartAccounts" float-label="CASH ACCOUNT"  />
                        <br>
    ...
    <q-modal v-model="modalPurchaseReceived">
        <div class="q-pa-md">
            ...
            <q-table ref="table" color="primary" :data="purchaseReceivedItems.items" :columns="columns" hide-bottom class="no-shadow">
                ...
                <template slot="body" slot-scope="props">
                    <q-tr :props="props">
                        ...
                        <q-td key="price" :props="props">
                            {{ props.row.pivot_price | currency('₱') }}
                        </q-td>
            ...
            <q-btn color="primary" @click="modalPurchaseReceived = false" label="Close" class="q-ma-sm" />
        </div>
    </q-modal>
</div>
</template>
...
export default {
    ...
    methods: {
        ...mapActions("transactions", ["setTax", "setTaxTypes"]),
        getPurchaseReceived(purchaseReceivedId) {
            if(purchaseReceivedId === ""){
               this.$q.notify({
                    color: "negative",
                    icon: "warning",
        ...
        store() {
            if (this.payee.payable_type === undefined || this.payee.payable_id === undefined) {
                this.$q.notify({
                    color: "negative",
                    icon: "warning",
            ...
            console.log(this.transaction.chart_account_id)
                    ...
                    if (headChange != null) {
                        Object.keys(vm.additionalItems).forEach(function (key) {
                            if (vm.additionalItems[key].id === headChange.id) {
                                if (vm.additionalItems[key].item_id === "") {
