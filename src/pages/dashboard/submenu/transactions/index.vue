    ...
    <div class="row">
        <div class="col-xs-6">
            <div class="q-display-1 q-mb-md">
                <q-select v-model="selectedEntity" :options="entities" float-label="Entity Type" clearable />
            </div>
        </div>
        <div class="col-xs-6">
            <div class="q-display-1 q-mb-md">
                <q-select filter v-model="selectedUserEntity" :options="userEntities" float-label="Entity Name" clearable />
            </div>
        </div>

    </div>
    <q-table ref="table" color="primary" :title="`${titleTransaction.name} Transactions`" :data="serverData" :columns="columns" :filter="filter" row-key="name" :pagination.sync="serverPagination" :rows-per-page-options="options" @request="request" :loading="loading">
