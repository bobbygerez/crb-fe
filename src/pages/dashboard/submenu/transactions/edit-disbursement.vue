        ...
        >
          ...
          <q-card-main>
            <div class="row">
              <div class="col-xs-4">
                <q-select
                  v-model="editTransaction.payee.payable_type"
                  filter
                  :options="entities"
              ...
              <div class="col-xs-4">
                <q-select
                  v-model="editTransaction.payee.payable_id"
                  filter
              ...
              <div class="col-xs-4">
                <q-input
                  v-model="editTransaction.checknumber"
            ...
            <div class="row">
              ...
              <div class="col-xs-3">
                <input-price
                  label="Vatable Sales"
                  :value="editTransaction.vatable_sales"
                  v-model="editTransaction.vatable_sales"
                ></input-price>
                <input-price
                  label="Zero Rated Sales"
                  :value="editTransaction.zerorated_sales"
                  v-model="editTransaction.zero_rated_sales"
                ></input-price>
              </div>
              <div class="col-xs-3">
                <input-price
                  label="VAT-Exempt Sales"
                  :value="editTransaction.vatexempt_sales"
                  v-model="editTransaction.vat_exempt_sales"
                ></input-price>
                <input-price
                  label="VAT Amount"
                  :value="editTransaction.vat_amount"
                  v-model="editTransaction.vat_amount"
                ></input-price>
              </div>
            </div>
          </q-card-main>
      ...
      <div class="col-xs-3">
        <div class="col-xs-8">
          ...
          >
            <q-card-main>
              <q-select
                v-model="editTransaction.chart_account_id"
                filter
              ...
              <input-price
                ...
                :value="editTransaction.total_discount"
                v-model="editTransaction.total_discount"
                class="q-ml-sm"
              ></input-price>
              <input-price
                label="Cash Account Balance"
                :value="editTransaction.total_amount"
                v-model="editTransaction.total_amount"
                class="q-ml-sm"
              ></input-price>
              <q-input
                v-model="editTransaction.remarks"
                type="textarea"
                float-label="Remarks"
                :max-height="150"
                rows="4"
            ...
            >
              <div class="col-xs-3">
                <q-btn
                  ...
                  flat
                  class="float-right"
                  @click="getPurchaseReceived(invoice.purchase_received_id)"
                >
  ...
  watch: {
      ...
      handler: function (after, before) {
        ...
        let changed = after.filter(function (p, idx) {
          return Object.keys(p).some(function (prop) {
            return p[prop] !== vm1.$data.oldAdditionalItems[idx][prop]
          })
        })

        if (changed.length === 1) {
          let entityItem = _.find(vm1.entityItems, {
          ...
          if (headChange != null) {
            Object.keys(vm1.additionalItems).forEach(function (key) {
              if (vm1.additionalItems[key].id === headChange.id) {
                vm1.additionalItems[key].price = entityItem.price
                vm1.additionalItems[key].desc = entityItem.label
                vm1.additionalItems[key].chart_account_id = entityItem.chart_account_id
                vm1.additionalItems[key].tax_type = entityItem.tax_type
                vm1.additionalItems[key].tax_type_id = entityItem.tax_type_id
                if (parseInt(headChange.qty) > 0) {
                  vm1.additionalItems[key].amount = parseFloat(parseInt(headChange.qty) * entityItem.price)
                } else {
                  vm1.additionalItems[key].amount = 0
                }
              }
            })
          }
    ...
    invoices: {
      handler: function (after, before) {
        ...
        let changed = after.filter(function (p, idx) {
          return Object.keys(p).some(function (prop) {
            return p[prop] !== vm1.$data.oldInvoices[idx][prop]
          })
        })
        if (changed.length === 1) {
          let purchaseReceived = _.find(vm1.purchaseReceived, {
            value: _.head(changed).purchase_received_id
          })
          let headChange = _.head(changed)
          Object.keys(vm1.invoices).forEach(function (key) {
            if (vm1.invoices[key].id === headChange.id) {
              vm1.invoices[key].date_due = purchaseReceived.date_due
              vm1.invoices[key].discount = purchaseReceived.discount
              vm1.invoices[key].amount_due = purchaseReceived.grand_total
              vm1.invoices[key].vatable_sales = purchaseReceived.vatable_sales
              vm1.invoices[key].vat_exempt_sales =
                purchaseReceived.vat_exempt_sales
              vm1.invoices[key].zero_rated_sales =
                purchaseReceived.zero_rated_sales
              vm1.invoices[key].vat_amount = purchaseReceived.vat_amount
              if (headChange.amount_paid > 0) {
                vm1.invoices[key].amount_paid = headChange.amount_paid
              } else {
                vm1.invoices[key].amount_paid = purchaseReceived.grand_total
              }
            }
          })
        }
