<template>
    <form @submit.prevent="add">
        <div class="row">
            ...
            <div class="col-4">
                <q-input outlined v-model="$v.chartAccount.account_code.$model" label="Account Code" class="q-ma-sm" :error="($v.chartAccount.account_code.$dirty && !$v.chartAccount.account_code.required) || !$v.chartAccount.account_code.numeric" bottom-slots error-message="Account Code must be a number and required." />
            </div>
            <div class="col-4">
...
export default {
  props: ['parentAccount', 'chartAccount', 'selectedChartAccount'],
  validations: {
    chartAccount: {
      name: {
        required
